...
class Trainer:
    ...
    def __parse_config(self):
        self.max_init = self.config.train["init"]
    ...
    def init_mean_model(self, dataloader):
        ce = nn.CrossEntropyLoss()
        optimizer = torch.optim.SGD(self.mean_model.parameters(), lr=self.lr / 100, weight_decay=5e-4)
        self.mean_model.train()
        for iter, (img, target) in enumerate(dataloader):
            ...
            loss = (ce(output1, target) + ce(output2, target) + ce(output3, target)) / 3
            loss.backward()
            optimizer.step()
            if iter >= self.max_init:
                break
        self.mean_model.set_trainable(False)
        print("mean model init finished!")
